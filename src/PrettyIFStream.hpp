// src:
// http://stackoverflow.com/questions/23400933/most-efficient-way-of-creating-a-progress-bar-while-reading-input-from-a-file
...
class ProgressBarBase {
 public:
  ProgressBarBase(int max) : max_(max) { print(0); }
 ...
 protected:
  ...
  inline void print(float prog) {
    ...
    std::cout << std::fixed << std::setprecision(2) << "\r   ["
              << std::string(cur, '#') << std::string(max_ + 1 - cur, ' ')
              << "] " << 100 * prog << "%";
...
class ProgressBar : public ProgressBarBase {
 public:
  ...
  inline void drawbar(float prog) { print(prog); }
};
